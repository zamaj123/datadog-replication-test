...
## Status
Design updated to conform to `INTERFACES.md`. `apps/storage` now requires ClickHouse configuration for the metrics read path and returns an explicit error instead of synthetic data when that dependency is absent. The canonical `metrics` table DDL and a local ClickHouse schema init script are now present, the metrics read path now uses exact nanosecond ClickHouse range boundaries while preserving raw-data behavior for auto-selected short ranges, and the HTTP server now allows the frontend dev origin to call the storage API with `X-Api-Key` over CORS. The env-metrics milestone now has real ClickHouse-backed `environments`, `services`, and `service summary` reads in storage, validated by service-aggregate tests, while the milestone plan documents the remaining contract clarification needed before implementing a version-breakdown read path. Broader Phase 2 implementation remains pending.
